...
#
# The way that VolScan works is that it collects all the crypto coin/token data for all USDT coin
# pairings that appear on Binance into user defined "scanning periods" which are varying numbers of minutes in length,
# each period then being split into the number of individual scans that makes up the period.
